...
class Engine(object):
	...
	def registerKiller(self, killer):
		'''
		passed a gracefulKiller instance from a plugin, register it
		if ANY plugin receives a gracefulKill request, it'll be passed to the daemon
		'''
	...
	def start(self):
		...
		try:
			#while True not in self.killers:
			...
			print "error occured during startup/loop"
...
class Plugin(object):
	...
	def load(self):
		...
		#also look for a graceful killer, if found, pass it up
		...
		killer=getattr(plugin, 'killer', None)
		if killer:
			self.logger.debug('found graceful killer in '+self._pluginName)
			self._engine.registerKiller(killer)
